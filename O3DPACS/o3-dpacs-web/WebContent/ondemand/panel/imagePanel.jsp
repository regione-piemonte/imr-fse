...
<a4j:outputPanel id="imageContainer" style="text-align: center;"
	...
	<rich:panel id="pannelloTest" style="text-align: center; "
		...
		<img id="wadoBigImage"
			src="<h:outputText value='#{selectedFilteredObject.wadoUrlForImage}'/>"
...
<a4j:form>
	<a4j:jsFunction name="nextImage"
		action="#{KosFilterManager.nextInstance}" reRender="imageContainer" />
	<a4j:jsFunction name="prevImage"
		action="#{KosFilterManager.prevInstance}" reRender="imageContainer" />
</a4j:form>
<h:panelGrid columns="1" styleClass="scrollerGroup">
	<a4j:form>
		<rich:inputNumberSlider id="imageChanger"
			value="#{KosFilterManager.currentInstancePosition}"
			styleClass="innerScroller" width="80%"
			maxValue="#{KosFilterManager.totalInstances}" step="1"
			minValue="1" showToolTip="true" enableManualInput="false" 
			valueChangeListener="#{KosFilterManager.valueChangeListener}"
			showInput="true">
	...
	<h:panelGrid id="grigliaSlider" columns="2" columnClasses="ex,none"
		...
		<h:outputText value="#{msg.wwidth}"
			rendered="#{not empty selectedFilteredObject.wwidth}" />
		<a4j:form id="windowWidthForm">
			<rich:inputNumberSlider id="windowWidth"
				value="#{selectedFilteredObject.wwidth}" width="600px"
				maxValue="#{selectedFilteredObject._maxWindowSize}"
		...
		<h:outputText value="#{msg.wcenter}"
			rendered="#{not empty selectedFilteredObject.wcenter}" />
		<a4j:form id="windowCenterForm">
			<rich:inputNumberSlider id="windowCenter"
				value="#{selectedFilteredObject.wcenter}" width="600px"
				maxValue="#{selectedFilteredObject._maxCenterPoint}"
				minValue="#{selectedFilteredObject._minCenterPoint}"
				step="1" showToolTip="true" enableManualInput="false"
				showInput="true"
				rendered="#{not empty selectedFilteredObject.wcenter}">
				<a4j:support event="onchange" reRender="pannelloTest" />
