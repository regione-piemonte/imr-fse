...
<f:view locale="#{sessionScope.locale}"
	...
	<head>
	<link rel="stylesheet" href="<h:outputText value="../css/style.css" />" />
	...
	<a4j:include viewId="/commons/messagePanel.jsp"></a4j:include>

	<div id="wrapper">
	<div id="contentTitle"><h:outputText value="#{msg.serieResults}" /></div>
	<div id="content"><h:panelGrid columns="2"></h:panelGrid> <h:panelGrid
		columns="2" styleClass="top" rowClasses="top" columnClasses="none,tmp">
		<rich:dataTable value="#{kosFilteredSeries.seriesMatched}" var="item"
			id="tableStudies" headerClass="headerTable" styleClass="tableResults"
			rowClasses="oddRow, evenRow"
			columnClasses="column10,column10,column10,none,none,none,none,none,none,none">
			<rich:toolTip>
				<span style="white-space: nowrap"> <h:outputText
					value="#{msg.description}: " /> <h:outputText
					value="#{item.seriesDescription}"></h:outputText> </span>
			</rich:toolTip>
			<rich:column rendered="#{item.inWarning}">
				<img src="../img/error.png" title="<h:outputText value="#{msg.seriesInWarning}" /> "/>
			</rich:column>
			<rich:column>
				<a4j:form>
					<a4j:commandLink data="#{item}"
						actionListener="#{KosFilterManager.selectFilteredSeries}"
		...
		<a4j:outputPanel id="images">
			...
			<rich:dataGrid id="imagesGrid"
				value="#{filteredObject.objectsMatched}" styleClass="adaptiveCell"
				var="item" align="center" columns="#{seriesResultsBackBean.thumbsPerRow}"
				elements="#{seriesResultsBackBean.thumbsPerPage}"
				rendered="#{filteredObject.objectsMatched != null}">
				<f:facet name="header">
					<h:form>
						<rich:datascroller
							rendered="#{filteredObject.objectsMatched != null}"
				...
				<h:outputLink value="#{item.wadoUrlForPDF}"
					...
					<h:graphicImage value="../img/lis_pdf.gif" width="80" height="83"
				...
				<a4j:form>
					<a4j:outputPanel id="thumbContainer"
						...
						<a4j:commandLink 
							actionListener="#{KosFilterManager.selectFilteredInstance}"
							data="#{item}"
							reRender="immagine"
							oncomplete="Richfaces.showModalPanel('BigImagePopup',{width:document.body.offsetWidth-50, height:window.innerHeight-50})">
							<img src="<h:outputText value="#{item.wadoUrlForImage}"/>"
								style="border-width: 0px;" />
						</a4j:commandLink>
					</a4j:outputPanel>
					<a4j:outputPanel id="thumContainerIfMpeg"
						...
						<a4j:commandLink 
							actionListener="#{KosFilterManager.selectFilteredInstance}"
							data="#{item}"
							rendered="#{sessionScope.isHttps != 'https'}" 
							reRender="immagine"
							oncomplete='Richfaces.showModalPanel("BigImagePopup",{width:800,height:600})'>
							<img src="../img/mpeg.jpg" style="border-width: 0px;" />
						</a4j:commandLink>
						<h:outputLink value="#{item.wadoUrlForMpeg}"
							rendered="#{sessionScope.isHttps == 'https'}">
							<img src="../img/mpeg.jpg"
								title="<h:outputText value="#{msg.download}"/>"
								style="border-width: 0px;" />
						</h:outputLink>
					</a4j:outputPanel>
				</a4j:form>
	...
	</h:panelGrid> <a4j:outputPanel id="immagine">
		<rich:modalPanel id="BigImagePopup"
			rendered="#{selectedFilteredObject != null}">
			<f:facet name="header">
				<h:panelGroup>
			...
			<a4j:outputPanel id="viewer">
				<a4j:include id="imagePanel" viewId="panel/imagePanel.jsp"
					rendered="#{selectedFilteredObject.viewer == 'imagePanel'}" />
				<a4j:include id="videoPanel" viewId="panel/videoPanel.jsp"
					rendered="#{selectedFilteredObject.viewer == 'videoPanel'}" />
				<a4j:include id="noPanel" viewId="panel/noplayerPanel.jsp"
					rendered="#{selectedFilteredObject.viewer == 'noPanel'}" />
				<a4j:include id="mpegVideoPanel" viewId="panel/mpegVideoPanel.jsp"
					rendered="#{selectedFilteredObject.viewer == 'mpegVideoPanel'}" />
			</a4j:outputPanel>
		</rich:modalPanel>
	</a4j:outputPanel></div>
